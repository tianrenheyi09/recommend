...
evals_results = {}
print('train')
lgb_model = lgb.train(lgb_params,lgbtrain,num_boost_round=1000,valid_sets=lgbvalid,categorical_feature=categorical_feature_list,early_stopping_rounds=50,verbose_eval=10)
...
aa = pd.DataFrame(data=gbdt_feats_vals, index=range(0, gbdt_feats_vals.shape[0]),columns=gbdt_columns)
ab = pd.DataFrame()
