...
from DCN_RE import DCN
from my_DCN_RE import my_DCN
...
# ############随机打乱划分训练集和验证集
np.random.seed(2018)
shuftle_index = np.random.permutation(len(cate_Xi_train))
train_idx = shuftle_index[0:int(0.08*len(cate_Xi_train))]
valid_idx = shuftle_index[int(0.08*len(cate_Xi_train)):int(0.1*len(cate_Xi_train))]
_get = lambda x,l:[x[i] for i in l]
...
dcn = DCN(**dcn_params)
############训练
my_dcn = my_DCN(**dcn_params)

my_dcn.fit(cate_Xi_train_, cate_Xv_train_, numeric_Xv_train_,y_train_, cate_Xi_valid_, cate_Xv_valid_, numeric_Xv_valid_,y_valid_)
